      ...
      <el-row :gutter="30">
        ...
        <el-col :span="19">
          <div class="content-right">
            ...
            <div>
              <el-table ref="tableRef" :data="data.userscoreList" size="large"
                ...
                <el-table-column label="评分值" prop="score">
                  <template #default="scope">
                    {{ scope.row.score }} 分
                  </template>
                </el-table-column>
                <el-table-column label="评分时间" prop="savedate" width="200"/>
...
const loadData = () => {
  //请求数据
  request.get('/user/userscore/list', {
    params: { pageNum: data.pageNum, pageSize: data.pageSize }
  }).then(res => {
    data.userscoreList = res.pageBean.list //评分列表
...
const doDelete = (selectIds, isBatch = false) => {
  ...
  //调用公共删除函数
  handleDeleteBatch(selectIds, '/user/userscore/doDelete', loadData)
}
